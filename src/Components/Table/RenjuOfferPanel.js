...
// button here — only the running count and a cancel. It also renders in 'placing' mode (the
// "Decline & place" arm) with a Cancel, so an accidental decline is recoverable BEFORE any
// stone is sent: reset returns the slice to idle, which re-shows the choice modal.
...
const UnconnectedRenjuOfferPanel = ({renjuUi, reset}) => {
   if (renjuUi.mode === 'placing') {
      return (
         <div style={{padding: '0.5rem'}}>
            <Typography variant="subtitle1">Decline & place — tap a point to play, or cancel</Typography>
