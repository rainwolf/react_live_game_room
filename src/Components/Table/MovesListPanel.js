      ...
      if (move_strs.length > 0) {
         move_cells.push(
            <TableCell key={0} align='center' onClick={() => props.goto_move(1)}>
               <Typography variant={(game.until === 1) ? 'h5' : 'h6'}
                           color={(game.until === 1) ? 'error' : 'textPrimary'}>
                  {move_strs[0]}
               </Typography>
            </TableCell>)
      }
      for (let i = 1; i < move_strs.length; i++) {
         move_cells.push(
            <TableCell key={i} align='center'
               ...
               <Typography variant={(game.until === 1 + 2 * i || game.until === 2 * i) ? 'h5' : 'h6'}
                           color={(game.until === 1 + 2 * i || game.until === 2 * i) ? 'error' : 'default'}>
                  {move_strs[i]}
               </Typography>
            </TableCell>)
      }
